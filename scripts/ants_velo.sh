...
if [[ $ISLARGEIMAGE -eq 1 ]];
  then
    ...
    SYNSHRINKFACTORS="2x2x1" # README: Dont downsample too much ...
    SYNSMOOTHINGSIGMAS="2x1x0vox" # instead, smooth more
  fi
# README - change this for your needs at the time
ffn=${OUTPUTNAME}padded.mha
mfn=${OUTPUTNAME}shifted.mha
ImageMath $DIM $ffn PadImage ${FIXEDIMAGES[0]} 10
ImageMath $DIM $mfn ShiftImageSlicesInTime $ffn 1
MOVINGIMAGES=($mfn)
FIXEDIMAGES=($ffn)
...
echo "--------------------------------------------------------------------------------------"
$COMMAND
CreateWarpedGridImage $DIM ${OUTPUTNAME}0Warp.nii.gz ${OUTPUTNAME}grid.nii.gz 1x1x0 5x5x0 1x1x0  # README one way to visualize 
###############################
#
